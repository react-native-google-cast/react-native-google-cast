    ...
    public void onStatusUpdated() {
        module.runOnUiQueueThread(new Runnable() {
            ...
            public void run() {
                ...
                module.emitMessageToRN(GoogleCastModule.MEDIA_STATUS_UPDATED, prepareMessage(mediaStatus));

                if (!playbackStarted && mediaStatus.getPlayerState() == MediaStatus.PLAYER_STATE_PLAYING) {
                    module.emitMessageToRN(GoogleCastModule.MEDIA_PLAYBACK_STARTED, prepareMessage(mediaStatus));
                    playbackStarted = true;
                }

                if (!playbackEnded && mediaStatus.getIdleReason() == MediaStatus.IDLE_REASON_FINISHED) {
                    module.emitMessageToRN(GoogleCastModule.MEDIA_PLAYBACK_ENDED, prepareMessage(mediaStatus));
                    playbackEnded = true;
                }
            }
        });
