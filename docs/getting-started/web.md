...
| `useCastSupported()` / `CastContext.isSupported()`                                | ✅ `false` in non-Chromium browsers and until the SDK announces itself; `true` once it has. **This is the cross-platform "can I cast here?" check — `getPlayServicesState()` is Android-only and reports `'success'` here even in Safari.** |
...
| `loadMedia` (single item + `queueData`), play/pause/stop/seek, stream volume/mute | ✅ — `queueData` crosses in full via `LoadRequest.queueData` (id, name, entity, type, repeatMode, containerMetadata, items, startIndex, startTime)                                                                                          |
| Media status stream incl. null-clear on unload                                    | ✅ (`Media.addUpdateListener`)                                                                                                                                                                                                              |
| `MediaStatus.videoInfo`                                                           | ✅ from `Media.videoInfo` (width/height/HDR type), when the receiver reports it                                                                                                                                                             |
